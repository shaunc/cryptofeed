...
class Binance(Feed, BinanceRestMixin):
    ...
    def _parse_symbol_data(cls, data: dict) -> Tuple[Dict, Dict]:
        ...
        for symbol in data["symbols"]:
            ...
            if symbol.get("contractType") == "PERPETUAL":
                stype = PERPETUAL
            elif symbol.get("contractType") in (
                "CURRENT_QUARTER",
                "NEXT_QUARTER",
            ):
                stype = FUTURES
    ...
    def _address(self) -> Union[str, Dict]:
        ...
        is_any_public = any(
            not self.is_authenticated_channel(chan)
    ...
    def _generate_token(self) -> str:
        url = self.rest_endpoints[0].route(
    ...
    def _check_update_id(self, std_pair: str, msg: dict) -> bool:
        ...
        if (
            ...
            self._reset()
            LOG.warning(
                "%s: Missing book update detected, resetting book", self.id
            )
            return True
    ...
    async def _snapshot(self, pair: str) -> None:
        ...
        resp = await self.http_conn.read(
            ...
            .route("l2book", self.sandbox)
            .format(pair, max_depth)
        )
